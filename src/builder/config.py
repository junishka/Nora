"""Builder — process-wide configuration and path sandboxing.

The researcher points Builder at a working directory (their project dir /
...
# Module-level state. Intentionally simple: Builder is a single-process CLI,
...
def resolve_in_cwd(user_path: str) -> Path:
    ...
    if not (resolved == cwd or resolved.is_relative_to(cwd)):
        raise PathEscapeError(
            ...
            f"the working directory {cwd}. Builder tools can only access "
