...
Lives at ``<cwd>/.builder/session_state.json`` and captures the facts
...
produce (everything is derivable from state Builder already keeps on
...
from builder.chat_history import read_turns
...
def write_session_state(
    ...
    *,
    ...
    if cwd is None or not cwd.is_dir():
        ...
        # orphan .builder/ folder behind and pretend a session
        # was there.
        return None
    ...
    #    no sizes (those live in ~/.builder-sessions listing).
    datasets: list[str] = []
    try:
    ...
    _atomic_write(cwd / ".builder" / SESSION_STATE_FILENAME, state)
    return state
...
def read_session_state(cwd: Path | None) -> SessionState | None:
    ...
    if cwd is None:
        return None
    path = cwd / ".builder" / SESSION_STATE_FILENAME
    if not path.exists():
        return None
    try:
