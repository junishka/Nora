"""Builder — researcher consent policy for schema exposure.
...
- Unknown or malformed entries fall back to the conservative
  ...
  prevent the researcher from using Builder — the default is safe.
"""
...
POLICY_FILE = Path(".builder") / "policy.json"
...
class BuilderPolicy:
    ...
    policy lives inside ``<cwd>/.builder/policy.json`` so paths are
...
def load_policy(cwd: Path) -> BuilderPolicy:
    ...
    lock the researcher out of using Builder, and the fallback is
    safe.
    """
    p = policy_path(cwd)
    if not p.is_file():
...
def save_policy(cwd: Path, policy: BuilderPolicy) -> None:
    """Persist ``policy`` to ``<cwd>/.builder/policy.json``.

    Creates the ``.builder`` directory if it doesn't already exist.
    """
    p = policy_path(cwd)
...
def get_max_depth(policy: BuilderPolicy, dataset_name: str) -> str:
...
def has_explicit_policy(policy: BuilderPolicy, dataset_name: str) -> bool:
