...
const MODEL_STORAGE_KEY = 'builder.model';
...
// follow-mode.
const THEME_STORAGE_KEY = 'builder.theme';
...
// OS theme change while Builder is open: keep the icon in sync
...
function replayEvent(evt) {
   ...
   * window.builder_event; replay feeds the same records back
...
function formatCwd(raw) {
   ...
   * chip. The full path —
   * ``/Users/bb/.builder-sessions/20260422T160059Z_f13630f4`` —
...
async function stageDataFile(file) {
  ...
  if (typeof window.pywebview.api.add_files_from_blobs !== 'function') {
    appendError('Restart Builder to drop files into the chat.');
    return;
  }
  const data = await new Promise((resolve, reject) => {
    const reader = new FileReader();
...
form.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (images.length > 0 && typeof window.pywebview.api.send_message_with_images === 'function') {
      ...
      appendError('Restart Builder to send images.');
      setSending(false);
      return;
    } else {
...
// R has no equivalent marker because ``source(builder.R)`` runs
...
function buildPolicyPopup(policy) {
   ...
   * dark mode. Button rows follow the Builder palette and are
...
function renderModelPopup() {
  ...
  const header = document.createElement('div');
  header.className = 'policy-popup-header';
  header.innerHTML = '<strong>Model</strong> — Claude model Builder uses for this session.';
...
async function setModel(modelId, silent) {
  ...
  if (typeof window.pywebview.api.set_model !== 'function') {
    toast('Restart Builder to enable model switching.', 'error', 'model');
    return;
  }
  try {
...
const SIDEBAR_COLLAPSED_KEY = 'builder.sidebarCollapsed';
const SIDEBAR_WIDTH_KEY = 'builder.sidebarWidth';
...
async function loadSessions() {
  ...
  if (typeof window.pywebview.api.list_sessions !== 'function') {
    renderSidebarMessage('Restart Builder to see past sessions.');
    return;
  }
  try {
    const res = await window.pywebview.api.list_sessions();
...
async function deleteSession(s, isCurrent) {
  ...
  if (typeof window.pywebview.api.delete_session !== 'function') {
    toast('Restart Builder to enable session deletion.', 'error');
    return;
  }
...
async function switchSession(path, isCurrent) {
  ...
  if (typeof window.pywebview.api.switch_session !== 'function') {
    ...
    toast('Restart Builder to enable session switching.', 'info');
    return;
  }
  try {
...
if (addFilesBtn) {
  addFilesBtn.addEventListener('click', async () => {
    ...
    if (typeof window.pywebview.api.add_files !== 'function') {
      toast('Restart Builder to enable mid-session file adding.', 'info');
      return;
    }
...
function friendlyAddFilesError(raw) {
  ...
  if (/not a valid file filter/i.test(s)) {
    return "Couldn't open the file picker. Try restarting Builder.";
  }
  ...
  if (/window not ready/i.test(s)) {
    return 'Builder is still starting up. Try again in a moment.';
  }
