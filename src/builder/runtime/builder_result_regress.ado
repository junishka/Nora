*! version 0.0.2  Builder runtime: emit a linear_regression payload from e().
*!
...
*! to the path in $BUILDER_RESULT_PATH so the Builder executor can pick
*! it up and route through the sanitizer.
*!
*! Usage:
*!   regress y x1 x2
*!   builder_result_regress, label("OLS income ~ edu+age")
*!
*! Notes:
...
program define builder_result_regress
    version 13
    syntax [, label(string) ]
    ...
    * (JSON disallows literal control chars in string values).
    ...
    if "`e(cmd)'" == "" {
        display as error "builder_result_regress: no regression results in memory. Run a regression command first."
        exit 198
    }

    local path : env BUILDER_RESULT_PATH
    if "`path'" == "" {
        display as error "BUILDER_RESULT_PATH not set. This script must be run through Builder."
        exit 198
    }
    ...
    * same scientific-notation formatter for JSON-safety.
    ...
    display as text "builder_result_regress: wrote result to " as result "`path'"
end
