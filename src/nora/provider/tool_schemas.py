...
_COMPOSE_RESULTS_DESC = (
    ...
    "the ``nora.result`` call). This is the minimal-friction form: "
    ...
    "verbose or needs renaming for the comparison context. A row "
    "dict may also carry ``\"session_path\"`` to pull that row's "
    "result from another session's store (paths come from "
    "``list_results_global``; gated by the same env var). Rows are "
    "resolved per (session_path, result_id) pair, so the same id "
    "from two sessions — e.g. this session's M1 next to another "
    "session's M1 — composes cleanly in one table.\n\n"
