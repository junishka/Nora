...
program define nora_plot_estimate_comparison
    ...
    * could otherwise have appended.
    ...
    if "`_nora_token'" == "" {
        display as error "nora_plot_estimate_comparison: NORA_RUN_TOKEN not set"
        exit 198
    }
    ...
    * unless the caller already stored them.
    ...
    capture noisily {
        ...
        * can't make the manifest line invalid.
        ...
        local jsonline `"{"file":"`_file'","kind":"estimate_comparison","label":"`lab'","format":"`_fmt'","_token":"`_nora_token'"}"'
        tempname mh
