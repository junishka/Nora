*! version 0.0.1  Nora runtime: emit frequency_table or crosstab from raw data.
*!
*! Pass one variable → 1-way frequency_table payload.
*! Pass two variables → 2-way crosstab payload.
*!
*! Usage:
*!   nora_result_tab state, label("State frequencies")
*!   nora_result_tab state gender, label("State x Gender")
*!
...
program define nora_result_tab
    version 13
    ...
    if `nvars' == 1 {
        _nora_tab_1way `varlist', label(`"`label'"')
    }
    else {
        _nora_tab_2way `varlist', label(`"`label'"')
    }
end
...
program define _nora_tab_1way
    ...
    * After contract: one row per distinct non-missing value, `_freq' the count.
    ...
    restore
    display as text "nora_result_tab (1-way): wrote result to " as result "`path'"
end
...
program define _nora_tab_2way
    ...
    * Close counts object and outer object.
    ...
    restore
    display as text "nora_result_tab (2-way): wrote result to " as result "`path'"
end
