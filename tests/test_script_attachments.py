...
# ---------------------------------------------------------------------------
# Session pinning — a drop belongs to the session it started in
# ---------------------------------------------------------------------------
#
# Both halves of the upload path used to resolve the session LATE:
#
#   * JS captured ``currentCwd`` after awaiting FileReader, and sent
#     no target at all, so the backend bound to whatever was focused
#     when the request landed.
#   * The backend wrote the bytes into a captured ``cwd`` but then
#     called ``_active_runner()`` to stage the script contents, so a
#     file written to A could be attached to B's next message.
#
# Neither needs malicious input — dropping a large file and clicking
# another session while the browser reads it is enough.

def _drop_payload(name: str, blob: bytes) -> list[dict]:
    """The shape JS sends: ``[{name, content (base64)}]``."""
    return [{
        "name": name,
        "content": "data:," + base64.b64encode(blob).decode("ascii"),
    }]


def _switch_focus_to(bridge: NoraBridge, tmp_path: Path) -> Path:
    """Open a second session and focus it, as if the researcher
    clicked it in the sidebar while the first file was still being
    read."""
    other = tmp_path / "session_b"
    other.mkdir()
...
def test_drop_lands_in_the_session_it_started_in(tmp_path: Path) -> None:
    bridge = _make_bridge(tmp_path)
    dropped_in = bridge.cwd.resolve()
    other = _switch_focus_to(bridge, tmp_path)

    csv = b"a,b\n1,2\n"
    res = bridge.add_files_from_blobs(
        _drop_payload("trial.csv", csv), str(dropped_in),
    )

    assert res["ok"] is True
    assert res["cwd"] == str(dropped_in)
    assert (dropped_in / "trial.csv").read_bytes() == csv
    assert not (other / "trial.csv").exists()


def test_dropped_script_attaches_to_the_session_that_received_it(
    tmp_path: Path,
) -> None:
    """The .py reproduction: bytes to A, contents to A's next
    message — not to whichever session the researcher moved to."""
    bridge = _make_bridge(tmp_path)
    dropped_in = bridge.cwd.resolve()
    other = _switch_focus_to(bridge, tmp_path)
    ...
    code = b"import pandas as pd\nprint('hi')\n"
    res = bridge.add_files_from_blobs(
        _drop_payload("analysis.py", code), str(dropped_in),
    )
    assert res["ok"] is True
    assert (dropped_in / "analysis.py").read_bytes() == code

    staged = bridge._runner_for_cwd(dropped_in).pending_script_attachments
    assert [s["name"] for s in staged] == ["analysis.py"]
    assert "import pandas" in staged[0]["content"]
    # The focused session must see nothing — it never received a file.
    assert bridge._runner_for_cwd(other).pending_script_attachments == []
    assert bridge._pending_script_attachments == []


def test_drop_into_a_closed_session_is_refused(tmp_path: Path) -> None:
    """If the target session was deleted while the browser read the
    file, refuse. Silently redirecting the drop into the focused
    session is the bug this whole path is fixing."""
    bridge = _make_bridge(tmp_path)
    gone = tmp_path / "deleted_session"
    gone.mkdir()

    res = bridge.add_files_from_blobs(
        _drop_payload("analysis.py", b"print(1)\n"), str(gone),
    )

    assert res["ok"] is False
    assert "no longer open" in res["reason"]
    assert not (gone / "analysis.py").exists()
    assert not (bridge.cwd / "analysis.py").exists()
    assert bridge._pending_script_attachments == []


def test_drop_without_a_target_still_uses_the_focused_session(
    tmp_path: Path,
) -> None:
    """Back-compat: older JS sends one argument. Falling back to the
    focused session is the pre-existing behaviour."""
    bridge = _make_bridge(tmp_path)
    res = bridge.add_files_from_blobs(_drop_payload("a.py", b"print(1)\n"))
    assert res["ok"] is True
    assert (bridge.cwd / "a.py").exists()
    assert len(bridge._pending_script_attachments) == 1


def test_js_captures_the_session_before_reading_the_file() -> None:
    """The JS half: ``uploadCwd`` must be read BEFORE the FileReader
    await and passed to the bridge. Captured after, it already holds
    the session the researcher switched to."""
    src = (Path(__file__).resolve().parent.parent
           / "src" / "nora" / "web" / "app.js").read_text(encoding="utf-8")
    body = src.split("async function stageDataFile(file) {", 1)[1]
    body = body.split("\n}\n", 1)[0]

    capture = body.index("const uploadCwd = currentCwd;")
    read = body.index("new FileReader()")
    assert capture < read, (
        "stageDataFile captures currentCwd after the FileReader read — "
        "a session switch during the read redirects the file"
    )
    # And the captured value has to reach the backend; capturing it
    # locally while the bridge binds to self.cwd fixes nothing.
    assert "], uploadCwd);" in body, (
        "stageDataFile does not pass uploadCwd to add_files_from_blobs"
    )
