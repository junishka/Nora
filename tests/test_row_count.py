...
def test_check_returns_none_when_no_source_given(dataset_100):
    ...
    msg = _check_row_count({"type": "linear_regression", "n": 50}, None)
    assert msg is None
    msg = _check_row_count({"type": "linear_regression", "n": 50}, "")
    assert msg is None


def test_check_returns_none_when_n_matches_source(dataset_100):
    msg = _check_row_count(
        {"type": "linear_regression", "n": 100}, "data.csv"
    )
    assert msg is None
...
def test_check_flags_shortfall(dataset_100):
    msg = _check_row_count(
        {"type": "linear_regression", "n": 80}, "data.csv"
    )
...
def test_check_flags_anomaly_when_analysis_n_too_big(dataset_100):
    msg = _check_row_count(
        {"type": "linear_regression", "n": 150}, "data.csv"
    )
    assert msg is not None
    assert "ROW COUNT ANOMALY" in msg


def test_check_silent_on_nonexistent_source(dataset_100):
    """Bad source path is silent; this is a best-effort signal, not a gate."""
    msg = _check_row_count(
        {"type": "linear_regression", "n": 50}, "nonexistent.csv"
    )
    assert msg is None


def test_check_silent_on_path_escape(dataset_100):
    msg = _check_row_count(
        {"type": "linear_regression", "n": 50}, "../../etc/passwd"
    )
    assert msg is None
...
def test_check_silent_when_effective_n_unknown(dataset_100):
    ...
    # unknown type
    msg = _check_row_count({"type": "weird"}, "data.csv")
    assert msg is None
    # suppressed crosstab
    msg = _check_row_count(
...
def test_check_applies_to_magnitude_table(dataset_100):
    ...
    msg = _check_row_count(payload, "data.csv")
    assert msg is not None
    assert "n=50" in msg
