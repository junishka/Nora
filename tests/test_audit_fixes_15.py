...
# ---------------------------------------------------------------------------
# 2. Plot-manifest consumers reject _token-bearing entries
# ---------------------------------------------------------------------------


def test_capture_plots_refuses_token_bearing_manifest(
    tmp_path: Path,
) -> None:
    """Build a run dir whose ``_nora_plots/manifest.jsonl`` still
    carries ``_token`` on some entries (the executor's filter
    didn't strip it — eg PlotManifestUnsanitizable). The runner's
    ``_capture_plots`` must skip the entire manifest, not just the
    token-bearing rows. Because the runner only gates on ``kind``,
    accepting other entries would let a forged
    ``kind="coefficients"`` row ride through.

    The function takes ``run_dir`` and stages plots on
    ``self.pending_plot_images``; we instantiate a minimal stub
    around it to inspect the result."""
    ...
    plots.mkdir()
    # A real PNG so the path-safety + read-bytes pass.
    (plots / "fig.png").write_bytes(
        b"\x89PNG\r\n\x1a\n" + b"\x00" * 64,
    )
    manifest = plots / "manifest.jsonl"
    # Token-bearing entry: the executor's filter would have
    # stripped this field; observing it here means something
    # bypassed the filter.
    manifest.write_text(
        json.dumps({
            "file": "fig.png", "kind": "coefficients",
            "_token": "leftover-or-forged",
        }) + "\n",
        encoding="utf-8",
    )
    ...
    assert runner.pending_plot_images == [], (  # type: ignore[attr-defined]
        "capture_plots accepted a manifest that still carries "
        "_token; the executor's filter would have stripped that "
        "field, so its presence means the filter didn't complete"
    )


def test_manifest_allowed_plot_kinds_refuses_token_bearing_manifest(
    tmp_path: Path,
) -> None:
    """Recall path: ``_manifest_allowed_plot_kinds`` is the gate
    ``_is_disclosure_safe_image`` consults. A manifest with any
    ``_token``-bearing entry must return ``None`` even when the
    basename exists in the manifest, so a later
    ``read_attached_file`` call can't retrieve the forged plot."""
    from nora.tools import _manifest_allowed_plot_kinds

    plots = tmp_path / "_nora_plots"
    plots.mkdir()
    manifest = plots / "manifest.jsonl"
    # Two entries: one looks legit (kind in allowlist), but a
    # SEPARATE entry still carries ``_token``. The whole manifest
    # is untrusted because the filter didn't strip the field.
    manifest.write_text(
        json.dumps({"file": "fig.png", "kind": "coefficients"}) + "\n" +
        json.dumps({
            "file": "other.png", "kind": "interaction",
            "_token": "leftover",
        }) + "\n",
        encoding="utf-8",
    )
