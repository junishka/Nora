...
The sanitizer recognises twelve analysis shapes. The 0.10.0 release
...
| `factor_decomposition` | ✓ `from_pca`; factor analysis via `nora$result(...)` | ✓ `from_pca`; factor analysis via `nora.result(...)` | ✓ `nora_result_factor` (PCA + factor with pcf/pf/ml/ipf extraction; reads loadings + eigenvalues from `e()`; ML-FA goodness-of-fit via `e(chi2_ms)` / `e(p_ms)` / `e(df_ms)` / `e(ll)`) | **shipped 0.10.0** |
